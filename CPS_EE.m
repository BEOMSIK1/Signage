...
for i = 1:iter
    ...
    for rf_index = 1:length(N_rf)
        ...
        ang = ang.';           % fixed angle
        %% Switch
        sw1 = zeros(N_tx,2^B,N_rf(rf_index));  % initialize
        ...
        ee_(i,rf_index) = SR(rf_index)/(P_t+N_rf(rf_index)*P_rf+(2^B)*P_ps*N_rf(rf_index)+(N_rf(rf_index)*N_tx)*P_sw+(N_rf(rf_index))*P_sp+N_tx*P_cb+P_bb);
