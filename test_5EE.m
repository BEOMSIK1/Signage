...
B = 2;         %         resolution (bit)    1bit 2bit 5bit(inf)
P_bb = 0.200;
P_sw = 0.0025;
P_sp = 0.020;
P_cb = 0.020;
...
for i = 1:iter
    ...
    for rf_index = 1:length(N_rf)
        ...
        for nt = 1:N_tx
            for nrf = 1:N_rf(rf_index)
                if V_(nt,nrf) < th
                    ...
                    elseif B == 5
                        F_hat(nt,nrf) = exp(1j*angle(V(nt,nrf)));  % inf
                        num_ps = num_ps+1;
                    end
                end
            end
        end
        npn(i,rf_index) = num_ps;
        ...
        SR5(rf_index) = SR5(rf_index) + mean( sum( log2( 1 + abs(S).^2 / (No*rx_ant) )) )/iter;
        %         num_error(i,rf_index) = biterr(data,rx_data);
        ee_(i,rf_index) = SR5(rf_index)/(P_t+N_rf(rf_index)*P_rf+num_ps*(P_ps+P_sw)+N_rf(rf_index)*P_sp+N_tx*P_cb+P_bb);
        %         ee_(i,rf_index) = SR(rf_index)/(P_t+N_rf(rf_index)*P_rf+N_rf(rf_index)*N_tx*((1-bet/100)*P_ps+P_sw)+P_bb);
        ps_percent(i,rf_index) = mean(npn(i,rf_index))*100/(N_tx*N_rf(rf_index));
