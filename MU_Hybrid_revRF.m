...
%           Wt = steer_precoding(model.fc, model.tx_ant, sel_angle(1:2,:)); % initial
%         Wr = steer_precoding(model.fc, model.rx_ant, sel_angle(3:4,:));
            Wt = exp(j.*random('normal',0,2*pi,tx_ant,N_rf));
        ...
        [Dsym, K] = ZF_precoding_WF(Dsym, He);
        Dsym = Dsym ./ K;
        
        Isym = ifft(Dsym, fft_len, 2) * sqrt(fft_len);
        tx_ofdm = [ Isym(:, fft_len - cp_len + 1 : end) Isym ];
        tx_ofdm = Wt * tx_ofdm;
        [rx_ofdm, No] = awgn_noise( model.FD_fading( tx_ofdm, H ), snr(n) );
        rx_ofdm = Wr.' * rx_ofdm;
        ...
        result_ber(n) = result_ber(n) + sum( sum( bit ~= rx_bit, 2 ), 1 ) / (data_len * N_s);
        rx_ofdm = Wr.' * model.FD_fading( tx_ofdm, H );
        rx_Isym = rx_ofdm(:, cp_len + 1 : fft_len + cp_len);
        rx_Dsym = fft(rx_Isym, fft_len, 2) / sqrt(fft_len);
        result_cap(n) = result_cap(n) + mean( sum( log2( 1 + abs(rx_Dsym).^2 / ( No * rx_ant ) ) ) );
