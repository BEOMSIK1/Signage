...
for i = 1:iter
    ...
    for rf_index = 1:length(N_rf)
        ...
        SRt(rf_index) = SRt(rf_index) + mean( sum( log2( 1 + abs(S).^2 / (No*rx_ant) )) )/iter;
        %         num_error(i,rf_index) = biterr(data,rx_data);
        
        ee_(i,rf_index) = SRt(rf_index)/(P_t+N_rf(rf_index)*P_rf+(2^B)*P_ps*N_tx+(N_rf(rf_index)*N_tx+sw2)*P_sw+N_tx*P_cb+P_bb);
...
%% Plot
figure(1)
plot(N_rf,ee, 'g-d','LineWidth',1.5)
...
figure(2)
plot(N_rf, SRt, 'g-d','LineWidth',1.5);
