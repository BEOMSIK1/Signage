clc, clear
%% SVD Analog precoder sub_connect
%% Parameters
fft_size = 64;
...
for i = 1:iter
    ...
    for snr_index = 1:length(snr)
        ...
        n = 1;
        %% RF
%         F_hat = ones(N_tx,N_rf);
        for l = 1:N_rf
            R = 0;         % mean channel (cov)
            for k = 1:fft_size
                H_(:,:) = H(k,:,:);
                R = R + (H_(:,(l-1)*(N_tx/N_rf)+1:l*(N_tx/N_rf))'*H_(:,(l-1)*(N_tx/N_rf)+1:l*(N_tx/N_rf)));
            end
            R = R/fft_size;
            [~,sig,V] = svd(R);
            F = V(:,1);
            F_hat((l-1)*(N_tx/N_rf)+1:l*(N_tx/N_rf),l) = exp(j.*angle(F));
        end
%         for l = 1:N_rf
%             rf_mean(l,l) = mean(F_hat((l-1)*(N_tx/N_rf)+1:l*(N_tx/N_rf),l));
%             spliter((l-1)*(N_tx/N_rf)+1:l*(N_tx/N_rf),l) = ones(N_tx/N_rf,1);
%         end
%         
%         F_hat = spliter * rf_mean ;
